...
- **[LLM_FACE_INTEGRATION.md](./LLM_FACE_INTEGRATION.md)** - 顔アプリとLLM/ナビゲーションの双方向連携仕様
...
* **判定閾値（距離）の自動変更：**
  * **`0.5 m`：** `stop`（一時停止）、`person_area`（人物探索エリア）、`wait_time`（待機）、`change_map`（地図切り替え）が設定されているウェイポイントで適用されます（現地で精密に合わせるため）。
...
* `/nav_control` (std_msgs/String) - 停止、再開、キャンセル操作
* `/stop` (std_msgs/Bool) - 人物検知側からの `False` を受けて人物探索状態を解除

**配信トピック:**
* `target_odom` (nav_msgs/Odometry) - アクティブなウェイポイント座標の配信
* `/stop` (std_msgs/Bool) - 一時停止時のストップコマンド配信
* `/person_area_arrived` (std_msgs/Bool) - 人物探索エリアへの到着状態（Transient Local）
* `/initialpose` (geometry_msgs/PoseWithCovarianceStamped) - 地図切り替え時の初期位置設定

**設定可能なパラメータ一覧 (Waypoint定義):**

| パラメータ名 | 型 | デフォルト値 | 説明 |
| :--- | :--- | :--- | :--- |
| `number` | `int` | **(必須)** | ウェイポイント番号（ID）。 |
| `x` | `float` | **(必須)** | 目標とするマップ上の X 座標 [m]。 |
| `y` | `float` | **(必須)** | 目標とするマップ上の Y 座標 [m]。 |
| `angle_radians` | `float` | **(必須)** | 目標姿勢の Yaw 角 [rad]。 |
| `threshold` | `float` | `-1.0` | 個別の到達判定距離 [m]。`2.5` 等を指定すれば2.5m手前で次を送り高速維持、`1.0` 等を指定すればインカットを防ぐために1.0m手前まで引き付けます。マイナスの場合は、通常の点であればデフォルト値（`2.0m`）が適用されます。 |
| `stop` | `bool` | `false` | `true` の場合、到達時に `/stop` トピックへ `True` をパブリッシュします。`/nav_control` に `resume`（または `start` / `continue`）が届くまで次へ進みません。 |
| `wait_time` | `float` | `0.0` | `0.0` より大きい場合、到達時に `/stop` トピックへ `True` をパブリッシュし、指定された秒数だけ**非ブロッキングでその場に待機**します。カウントダウン終了後、自動的に `/stop` へ `False` を送り、走行を再開します。 |
| `person_area` | `bool` | `false` | 人物探索エリア。到着時に `/stop=True` と `/person_area_arrived=True` を配信します。実機は停止しますが、Nav2には次のゴールを送信して維持します。人物検知側が `/stop=False` を配信すると走行が再開し、`/person_area_arrived` も `False` に戻ります。 |
| `change_map` | `str` | `""` | 切り替え先の地図名。指定がある場合、到達時に `change_map_simple.sh` を呼び出して**地図を動的に切り替えます。** 切り替え後、新しい地図と同名のウェイポイントファイル（`<地図名>.yaml`）を自動的にロードし、インデックス0から走行を再開します。また、新マップの最初のウェイポイント座標で `/initialpose` が発行され、自己位置が初期化されます。 |
...
waypoints:
  ...
  - number: 4
    ...
    y: 3.0
    angle_radians: 1.57
    person_area: true   # 人物探索地点：/stop=falseを受けるまで実機だけ停止します
```
