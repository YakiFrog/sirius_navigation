...
graph TD
    ...
    CheckCorr -->|"No"| RuleParse{"ローカルルールで確定できるか?<br>例: '前に行って', '左向いて', '1周して', '行け'"}
    
    %% ローカルルール
    RuleParse -->|"Yes (定型 / 数値でも確定可能)"| RuleParseOut["ローカルパース結果をコマンド列へ変換"]
    RuleParse -->|"No"| LLMPath
    
    %% LLM処理
    LLMPath --> QueryLLM["LM Studio API ポート1234 へ問い合わせ"]
    QueryLLM --> ParseJSON["生成されたJSONコマンドシーケンスをパース"]
    ParseJSON --> Sanitizer["コマンドサニタイザーで型/方向/単位を補正"]
    RuleParseOut --> Sanitizer
...
* **移動コマンド (標準値で動作)**:
  ...
  * その場旋回: `「その場で旋回」` `「回転して」` `「1周して」` など（360度回転）
* **速度調整**:
...
* 例: `「3m前に行って、そこで右に90度回って」`
  * 距離の単位（m、センチメートル）や、角度（度、回転、`1周` など）をローカルルールで優先的にパースします。
  * 曖昧でルールが決めきれないときだけ LLM が補助します。
...
## 🏗️ モジュール構造とディレクトリレイアウト

プログラムの肥大化を防ぎ、保守性を極めて高く保つために、本来1つのファイルだった `LlmDynamicGoal` ノードは、単一責任の原則 (Single Responsibility Principle) に従い、機能ごとに別々のクラスモジュールへと分割・整理されています。

* **[`llm_dynamic_goal.py`](./llm_dynamic_goal.py)**: 各サブモジュールをインスタンス化し、ROS 2 のパブリッシャー・サブスクライバー、タイマーやコールバックを仲介するオーケストレーターです。他のスクリプトとの互換性を保つために Python プロパティ (Getter/Setter) を介してサブモジュールの変数をバインドしています。
* **[`modules/llm_client.py`](./modules/llm_client.py)**: LM Studio との通信、Verifier APIの実行、プロンプトの構築、会話履歴の管理を担当します。
* **[`modules/landmark_manager.py`](./modules/landmark_manager.py)**: ランドマーク情報のYAMLロード、RViz2 向け Marker / MarkerArray の発行を担当します。日本語名からローマ字（Romaji）/英字への変換ロジックを含み、RViz2でのテキストのズレ・ブレ対策を実装しています。
* **[`modules/nav_controller.py`](./modules/nav_controller.py)**: `/spin` アクション、Nav2パラメータの動的な更新（SetParameters サービス）、目標値（`/goal_pose`）のパブリッシュやナビゲーションキャンセルを担当します。
* **[`modules/teleop_handler.py`](./modules/teleop_handler.py)**: `/cmd_vel` への Twist 速度指令の配信、ローカルコストマップからの四方障害物距離の判定、および障害物接近時の減速・緊急停止ロジックを統合した「補助付き前後移動（Assisted Drive）」を担当します。
* **[`modules/command_executor.py`](./modules/command_executor.py)**: 複数指示のコマンドキュー状態管理、表情パラメータの標準化、コマンドキューのシリアル実行順序制御を担当します。
* **[`modules/http_server.py`](./modules/http_server.py)**: 外部の Docker や Web インターフェースなどから `/instruction` エンドポイントで指示を受け取るためのマルチスレッド HTTP サーバーです。
